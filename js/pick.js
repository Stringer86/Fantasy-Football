...
function randomPick() {
  ...
  if (pick <= .499) {
    ...
    $player1.text('\u2714');
    $player2.text('\u2718');
    $container.append(`<div class="row"><div class="col l12 center"><h5>Stop worrying and pick the ${$name1}!</div></div>`);
  } else {
    $player2.addClass("check");
    $player1.addClass("redX");
    $player2.text('\u2714');
    $player1.text('\u2718');
    $container.append(`<div class="col l12 center"><h5>Stop worrying and pick the ${$name2}!</div>`);
  }
}

function createReset() {
  $container.append(`<div class="row"><div class="s4"></div><div class="col l4 m5 s12 center"><a class="#1565c0 blue darken-3 btn" id="button2" onClick="window.location.reload()">New decision</a></div><div class="s4"></div>`);
}
